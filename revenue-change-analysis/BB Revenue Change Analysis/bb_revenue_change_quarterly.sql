create temp table bb_base_quarterly
as

with dt_array as (
select      *
from unnest(generate_date_array('2018-10-01',current_date(), interval 1 quarter)) as period_start
)

,dates as (
select      period_start
            ,last_day(period_start,quarter) as event_dt
from        dt_array
where       last_day(period_start,quarter) <= current_date()        
)
...
create or replace table `skyuk-uk-per-sub-economic-poc.Revenue_Change_Analysis.BB_Base_Quarterly`
as
...
create or replace table `skyuk-uk-per-sub-economic-poc.Revenue_Change_Analysis.BB_Rev_Change_Quarterly_Account`
as
...
select      account_number
            ...
            ,bb_tech_category
            ,bb_speed
from        `skyuk-uk-per-sub-economic-poc.Revenue_Change_Analysis.BB_Base_Quarterly`
where       product = 'BB'             
)
...
select      distinct
            account_number
            ,last_day(eow,quarter) as event_dt

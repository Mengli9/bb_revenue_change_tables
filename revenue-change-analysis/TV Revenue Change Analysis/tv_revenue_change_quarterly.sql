create temp table tv_base_quarterly
as

with dt_array as (
select      *
from unnest(generate_date_array('2019-09-01',current_date(), interval 1 quarter)) as period_start
)

,dates as (
select      last_day(period_start,quarter) as event_dt
from        dt_array
...
create or replace table `skyuk-uk-per-sub-economic-poc.Revenue_Change_Analysis.TV_Base_Quarterly`
as
...
create or replace table `skyuk-uk-per-sub-economic-poc.Revenue_Change_Analysis.TV_Rev_Change_Quarterly_Account`
as
...
from        `skyuk-uk-per-sub-economic-poc.Revenue_Change_Analysis.TV_Base_Quarterly`
